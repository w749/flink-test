...
public class OutJoinTest implements Base {
    ...
    public static void main(String[] args) {
        orderDataStream.coGroup(rateDataStream)
                        ...
                        .apply((CoGroupFunction<Order, Rate, Result>) (first, second, out) -> first.forEach(order ->
                                second.forEach(rate ->
                                        out.collect(new Result(order.getOrderTime(), order.getPrice() * rate.getRate(), order.getItem()))))).print();
